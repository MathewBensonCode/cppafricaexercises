...
std::ostream& operator<<(std::ostream& stream,
    ...
    []<std::size_t... Indices>(std::ostream& stream, const auto& values,
        ...
        const auto top_and_bottom_edge =
            fmt::format("{:*^{}}\n", '*', max_width + 2);
        // Write everything.
        ((stream << top_and_bottom_edge)
         << ...
         << fmt::format("*{: >{}}*\n", std::get<Indices>(values), max_width))
            << top_and_bottom_edge;
...
int main(int, char*[]) {
    ...
    // **********************
    std::cout << box_of_values(
        min_value<std::int8_t>, max_value<std::uint8_t>,
